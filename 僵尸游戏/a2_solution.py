    ...
    def move_entity(self, start: Position, end: Position) -> None:
        ...
        * If there is an entity at the given end position, replace that entity
        ...
        Examples:
            ...
            Player()
        """
        if start != end and self.in_bounds(start) and self.in_bounds(end):
...
class TrackingZombie(Zombie):
    ...
    def _directions(
        ...
        def distance(direction):
            ...
            return new_position.distance(target)
...
class Inventory:
    ...
    def step(self) -> None:
        ...
        When this method is called, the lifetime of every item stored within
        the inventory should decrease. Any items in the inventory that have
        exceeded their lifetime should be removed.
        """
        new_items = []
...
class HoldingPlayer(VulnerablePlayer):
    ...
    def infect(self) -> None:
        ...
        immune to becoming infected if they are holding garlic.
        """
...
class AdvancedTextInterface(TextInterface):
    ...
    def _first_in_direction(
            self, grid: Grid, start: Position, offset: Position
        ...
        if action == FIRE:
            ...
            if player.get_inventory().contains(CROSSBOW):
                ...
                if direction in DIRECTIONS:
                    ...
                    first = self._first_in_direction(game.get_grid(),
                                                     start, offset)
...
def task1(filename: str) -> Game:
    """Return a game corresponding to the task 1 functionality."""
    loader = BasicMapLoader()
    grid = loader.load(filename)
    return Game(grid)


def task2(filename: str) -> Game:
    """Return a game corresponding to the task 2 functionality."""
    loader = IntermediateMapLoader()
    grid = loader.load(filename)
    return IntermediateGame(grid)


def task3(filename: str) -> Game:
    """Return a game corresponding to the task 3 functionality."""
...
def main() -> None:
    ...
    # app = TextInterface(game.get_grid().get_size())
